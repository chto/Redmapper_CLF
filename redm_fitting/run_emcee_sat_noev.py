...
if __name__ == '__main__':
    ...
    start = [ 4.1, 0.836, 22.9, 0.034, 2.5, -.85, 4.]
    #Readjust to starting guess with NO REDSHIFT EVOLUTION INCLUDED
    #ln phi0 A_phi lnLs0 A_s alpha s
    #start = [ 4.1, 0.836, 22.9+2.5*np.log(1+float(input_params['hmf_files'][0][0])), 0.034, -0.85, 2.]
    zval = float(input_params['hmf_files'][0][0])
    start = [ 3.9+0.6*np.log(1+zval), 0.836, 23.2+2.*np.log(1+zval), 0.034, -0.9, 2.]
    nparam = len(start)
    ...
    err_guess = [0.1, 0.1, 0.01, 0.1, 0.1, 0.1]
    ...
    sampler = emcee.EnsembleSampler(nwalkers, nparam, fit_psat.psat_likelihood_conv_noev, args=[lm_param,cen_param,mass_param,x,y,covar_all,myz,psat_arg_set])
    ...
    f = open(outdir+"chain_sat_z_"+input_params['hmf_files'][0][0]+"_"+input_params['hmf_files'][0][1]+".dat",'w')
    f.close()
    
    for result in sampler.sample(pos, iterations=nsteps, storechain=False):
        position = result[0]
        prob = result[1]
        f = open(outdir+"chain_sat_z_"+input_params['hmf_files'][0][0]+"_"+input_params['hmf_files'][0][1]+".dat",'a')
