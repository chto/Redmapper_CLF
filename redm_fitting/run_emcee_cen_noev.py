...
if __name__ == '__main__':
    ...
    l_zmin = input_params['hmf_files'][0][0]
    l_zmax = input_params['hmf_files'][0][1]
    ...
    #sigma_L r lnL0 A_L s
    start = [0.36, 0.2, 24.74 + 1.21*np.log(1+float(l_zmin)), 0.40, 2.]
    if use_lm_param_file:
        start = [lm_param_dat[0,0],lm_param_dat[0,1],lm_param_dat[0,2], 0.36,
                 0.2, 24.74 + 1.21*np.log(1+float(l_zmin)), 0.40, 2.]
    print start
    ...
    if not use_lm_param_file:
        sampler = emcee.EnsembleSampler(nwalkers, nparam, fit_plm.plm_like_cen_noev, 
                                        args=[x,y,covar_all,myz,lambda_big,volume,
                                              sigma2_R,npoints])
    else:
        sampler = emcee.EnsembleSampler(nwalkers, nparam, fit_plm.plm_like_cen_noev_with_AL_prior, 
    ...
    f = open(outdir+"chain_cen_z_"+l_zmin+"_"+l_zmax+".dat",'w')
    f.close()
    
    for result in sampler.sample(pos, iterations=nsteps, storechain=False):
        position = result[0]
        prob = result[1]
        f = open(outdir+"chain_cen_z_"+l_zmin+"_"+l_zmax+".dat",'a')
